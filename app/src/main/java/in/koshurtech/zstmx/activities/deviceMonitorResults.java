...
public class deviceMonitorResults extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        swipeRefreshLayout = (SwipeRefreshLayout) findViewById(R.id.resultsViewSRL);
        recyclerView = (RecyclerView) findViewById(R.id.resultsRV);
        resultViewAdapter = new resultViewAdapter(resultViewArrayList, getApplicationContext(), deviceMonitorResults.this);
        linearProgressIndicator = (LinearProgressIndicator) findViewById(R.id.resultsLoading);
        linearProgressIndicator.setVisibility(View.VISIBLE);



        LinearLayoutManager layoutManager = new LinearLayoutManager(deviceMonitorResults.this);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            for (File f : files
                ...
                try {
                    ...
                    String body = "Service ID : "+jsonArray.getJSONObject(7).getString("serviceID")+"\n" +
                            "Parameters : "+jsonArray.getJSONObject(7).getString("parameters")+"\n" +
                            "Start Time : "+time(jsonArray.getJSONObject(7).getString("startTime"))+"\n" +
                            "End Time : "+time(jsonArray.getJSONObject(7).getString("endTime"));
                    resultViewArrayList.add(new resultView(f.getName(),body,false));
